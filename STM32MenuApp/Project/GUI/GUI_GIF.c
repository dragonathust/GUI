...
#ifdef MEM_DYNAMIC
#define ACODE_SIZE ( (1 << MAX_NUM_LWZ_BITS)*2)
#define APREFIX_SIZE ( (1 << MAX_NUM_LWZ_BITS))
#define ADECOMPBUFFER_SIZE 3000
#endif
...
#ifdef MEM_DYNAMIC
  GUI_HMEM h_aCode = 0;
  GUI_HMEM h_aPrefix = 0;
  GUI_HMEM h_aDecompBuffer = 0;
#endif  
...
static void _Memory_Free(void)
{
  	GUI_ALLOC_Free(h_aCode);
  	GUI_ALLOC_Free(h_aPrefix);
  	GUI_ALLOC_Free(h_aDecompBuffer);
	h_aCode=0;
	h_aPrefix=0;
	h_aDecompBuffer=0;
}

static int _Memory_Init(void)
{
      if(!h_aCode)
	  h_aCode = GUI_ALLOC_AllocNoInit(ACODE_SIZE);

      if(!h_aPrefix)
	  h_aPrefix = GUI_ALLOC_AllocNoInit(APREFIX_SIZE);

      if(!h_aDecompBuffer)
	  h_aDecompBuffer = GUI_ALLOC_AllocNoInit(ADECOMPBUFFER_SIZE);

      if(!(h_aCode&&h_aPrefix&&h_aDecompBuffer))
      	{
	_Memory_Free();
  	return 1;
      	}
...
#ifdef MEM_DYNAMIC
_LZWContext.aCode=(short*)GUI_ALLOC_h2p(h_aCode);
_LZWContext.aPrefix=(U8*)GUI_ALLOC_h2p(h_aPrefix);
_LZWContext.aDecompBuffer=(U8*)GUI_ALLOC_h2p(h_aDecompBuffer);
#endif
...
static int _GetNextByte(void) {
  ...
  while ((Code = _GetNextCode()) >= 0) {
    if (Code == _LZWContext.ClearCode) {
      ...
      /* Clear the tables */
#ifdef MEM_DYNAMIC	  
      GUI__memset((U8 *)_LZWContext.aCode, 0, ACODE_SIZE);
#else
...
#ifdef MEM_DYNAMIC	 	  
      if ((_LZWContext.sp - _LZWContext.aDecompBuffer) >= ADECOMPBUFFER_SIZE) {
#else
